        ...
        if ( ref( $argv{name} ) =~ /RR$/o ) {
            ...
            if ( my $ip = Ipblock->search( address => $argv{name} )->first ) {
                if ( $ip->interface && ( $dev = $ip->interface->device ) ) {
                    ...
                    $logger->debug(
                        sub {
                            "Device::search: Address $argv{name} exists but "
                ...
                $logger->debug(
                    sub { "Device::search: $argv{name} not found in DB" } );
            }
        }
        if ( !$foundname ) {
            ...
            if ( $argv{name} =~ /\D+/ ) {
                ...
                if ( my @rrs = RR->search( name => $argv{name} ) ) {
                    if ( scalar @rrs == 1 ) {
                        ...
                        if ( my @ips =
                            Netdot->dns->resolve_name( $argv{name} ) )
                        {
                            foreach my $ip (@ips) {
                                $logger->debug(
                                    sub {
"Device::search: $argv{name} resolves to $ip";
                                    }
                                );
                                if ( my $ip =
                                    Ipblock->search( address => $ip )->first )
                                {
                                    if ( $ip->interface
...
sub search_address_live {
    ...
    $class->throw_fatal(
        "Device::search_address_live: Cannot proceed without a MAC or IP")
    ...
    foreach my $dev (@arp_devs) {
        ...
        $dev->_netdot_rebless();
        eval {
            $cache = $class->_exec_timeout( $dev->fqdn,
                sub { return $dev->get_arp() } );
        };
        $logger->debug($@) if $@;
        if ($cache) {
    ...
    if ( $argv{mac} ) {
        ...
        foreach my $dev (@fwt_devs) {
            ...
            eval {
                $fwt = $class->_exec_timeout( $dev->fqdn,
                    sub { return $dev->get_fwt(%args) } );
            };
            $logger->debug($@) if $@;
        ...
        my @ordered =
          sort { $switchports{$a} <=> $switchports{$b} } keys %switchports;
    ...
    if ( Ipblock->matches_ip($host) ) {
        ...
        if ( my $ipb = Ipblock->search( address => $host )->first ) {
            if ( $ipb->interface && ( my $dev = $ipb->interface->device ) ) {
                $logger->debug(
"Device::assign_name: A Device with IP $host already exists: "
                      . $dev->get_label );
                return $dev->name;
            }
        }
        ...
        if ( defined $host && ( @ips = Netdot->dns->resolve_name($host) ) ) {
            my $str = join ', ', @ips;
            $logger->debug(
                sub { "Device::assign_name: $host resolves to $str" } );
        }
        else {
            $logger->debug(
                sub { "Device::assign_name: $host does not resolve" } );
        }
    }
    my $fqdn;
    my %args;
    ...
    if (@ips) {
        ...
        foreach my $i (@ips) {
            if ( $fqdn = Netdot->dns->resolve_ip($i) ) {
                $logger->debug(
                    sub { "Device::assign_name: $i resolves to $fqdn" } );
                if ( my $rr = RR->search( name => $fqdn )->first ) {
                    $logger->debug(
                        sub {
"Device::assign_name: RR $fqdn already exists in DB";
                        }
                    );
                    return $rr;
                }
                ...
                $logger->debug(
                    sub { "Device::assign_name: $i does not resolve" } );
            }
        }
    }
...
sub insert {
    ...
    if ( !exists( $argv->{name} ) ) {
        $class->throw_fatal(
            'Model::Device::insert: Missing required arguments: name');
    }
    ...
    if ( my $dbdev = $class->search( name => $devtmp{name} )->first ) {
        $logger->debug(
            sprintf(
                "Device::insert: Device %s already exists in DB as %s",
...
sub manual_add {
    ...
    if ($ip) {
        ...
        $ipb->update(
            { status => "Static", interface => $int, monitored => 1 } );
    ...
    if ($sinfo) {
        ...
        if ( ref($self) ) {
            if ( $self->snmp_target ) {
                ...
                $logger->debug(
                    sub {
"Device::get_snmp_info: Using configured target address: $args{host}";
                    }
                );
            }
            else {
            ...
            $self->throw_fatal(
'Model::Device::get_snmp_info: Missing required parameters: host'
            ) unless $args{host};
        }
        ...
        foreach my $arg (
            qw( communities version timeout retries sec_name sec_level
    ...
    $dev{community}    = $sinfo->snmp_comm
      if ( defined $sinfo->snmp_ver && $sinfo->snmp_ver != 3 );

    my $name_src =
      ( $self->config->get('IFNAME_SHORT') eq '1' )
      ? 'orig_i_name'
      : 'i_description';
    ...
    if ( defined $dev{sysobjectid} ) {
        ...
        if ( exists( $IGNORED{ $dev{sysobjectid} } ) ) {
            $logger->info(
                sprintf(
"%s Product id %s ignored per configuration option (IGNOREDEVS)",
    ...
    if ( $hashes{'e_descr'} ) {
        ...
        if ( defined $dev{_sclass} && $dev{_sclass} =~ /Airespace/o ) {
            ...
            $first_idx =
              ( sort { $a <=> $b } values %{ $hashes{'e_index'} } )[0];
        }
    ...
    $dev{syslocation} =
      $class->rem_lt_sp( $dev{syslocation} )    # Remove white space
      if ( $dev{syslocation} );
            ...
            if ( defined $dev{stp_type} && $dev{stp_type} ne 'unknown' ) {
                ...
                if ( $dev{stp_type} eq 'ieee8021d' || $dev{stp_type} eq 'mst' )
                {
                    ...
                    if ( $dev{stp_type} eq 'mst' ) {
                        ...
                        my $mapping = join ', ', map {
                            sprintf( "%s=>%s", $_, $dev{stp_vlan2inst}->{$_} )
                        } keys %{ $dev{stp_vlan2inst} };
                        $logger->debug(
                            sub {
"Device::get_snmp_info: $args{host} MST VLAN mapping: $mapping";
                            }
                        );
                 ...
                 # the STP standard info for at least one vlan on that instance.
                        ...
                        if ( $sinfo->cisco_comm_indexing() ) {
                            while ( my ( $vid, $mst_inst ) =
                                each %{ $dev{stp_vlan2inst} } )
                            {
                                ...
                                eval {
                                    my $vsinfo =
                                      $self->_get_cisco_snmp_context_session(
                                        sinfo => $sinfo,
                                        vlan  => $vid
                                      );
                                ...
                                if ( my $e = $@ ) {
                                    $logger->error(
"$args{host}: SNMP error for VLAN $vid: $e"
                                    );
                                    next;
                                }
                                foreach my $method ( keys %$stp_p_info ) {
                                    $dev{stp_instances}{$mst_inst}{$method} =
                                      $stp_p_info->{$method};
                                }
                                foreach my $field ( keys %$i_stp_info ) {
                                    foreach
                                      my $i ( keys %{ $i_stp_info->{$field} } )
                                    {
                                        $dev{interface}{$i}{$field} =
                                          $i_stp_info->{$field}->{$i};
                                    }
                                }
        ...
        if ($name) {
            if ( my $ifreserved = $self->config->get('IFRESERVED') ) {
                if ( $name =~ /$ifreserved/i ) {
                    $logger->debug(
                        sub {
"Device::get_snmp_info: $args{host}: Interface $name "
        ...
        if ( my $vm = $hashes{'i_vlan_membership'}->{$iid} ) {
            foreach my $vid (@$vm) {
                ...
                if ( exists $IGNOREDVLANS{$vid} ) {
                    $logger->debug(
                        sub {
"Device::get_snmp_info: $args{host} VLAN $vid ignored "
        ...
        foreach my $vid ( keys %{ $dev{interface}{$iid}{vlans} } ) {
            ...
            $dev{interface}{$iid}{vlans}{$vid}{vname} = $vname
              if defined($vname);
            ...
            if ( $dev{stp_type} ) {
                if ( $dev{stp_type} eq 'mst' ) {
                    ...
                    $dev{interface}{$iid}{vlans}{$vid}{stp_instance} =
                      $dev{stp_vlan2inst}->{$vid} || 0;
                }
                elsif ( $dev{stp_type} =~ /pvst/i ) {
    ...
    while ( my ( $ipt, $iid ) = each %{ $hashes{'ip_index'} } ) {
        ...
        if ( my $mask = $hashes{'ip_netmask'}->{$ip} ) {
            my ( $subnet, $len ) = Ipblock->get_subnet_addr(
                address => $ip,
                prefix  => $mask
            );
    ...
    while ( my ( $key, $val ) = each %{ $hashes{'ipv6_addr_prefix'} } ) {
       ...
       # It seems that for nexus the ifIndex id is always greater than 150000000
        ...
        if ( $iid && $addr && $pfx && $len ) {
            ...
            # What happened?
            $logger->warn(
                "$args{host}: Unrecognized ipv6_addr_prefix entry: $key => $val"
            );
        }
    }
    ...
    unless ($ignore_link_local) {
        ...
        while ( my ( $key, $val ) = each %$ipv6_index ) {
            if ( $key =~ /^.+\.((?:\d+\.){15}\d+)$/o ) {
                ...
                # What happened?
                $logger->warn(
                    "$args{host}: Unrecognized ipv6_index entry: $key => $val");
            }
        }
    }
...
# only proceed if we were told to discover peers, either directly or in the config file
    if ( $args{bgp_peers} || $self->config->get('ADD_BGP_PEERS') ) {
        ...
        foreach my $peer ( keys %{ $hashes{'bgp_peers'} } ) {
            ...
            unless ( $dev{bgp_peer}{$peer}{bgppeerid} =
                $hashes{'bgp_peer_id'}->{$peer} )
            {
    ...
    $logger->debug(
        sub {
"Device::get_snmp_info: Finished getting SNMP info from $args{host}";
        }
    );
    return \%dev;
}
...
sub snmp_update_all {
    ...
    $logger->info(
        sprintf(
            "All Devices updated. %d devices in %s",
...
sub snmp_update_block {
    ...
    my $blocks;
    $class->throw_fatal(
"Model::Device::snmp_update_block: Missing or invalid required argument: blocks"
      )
    ...
    $logger->info(
        sprintf(
            "Devices in $blist updated. %d devices in %s",
            $device_count, $class->sec2dhms( $end - $start )
        )
    );
...
sub snmp_update_from_file {
    ...
    my $file;
    $class->throw_fatal(
        "Model::Device::snmp_update_from_file:Missing required argument: file")
    ...
    $logger->info(
        sprintf(
            "Devices in %s updated. %d devices in %s",
...
sub discover {
    ...
    my $name = $argv{name}
      || $class->throw_fatal(
        "Model::Device::discover: Missing required arguments: name");
    ...
    if ( $dev = Device->search( name => $name )->first ) {
        $logger->debug(
            sub { "Device::discover: Device $name already exists in DB" } );
        ...
        unless ($info) {
            ...
            $info = $class->_exec_timeout(
                $name,
                sub {
                    return $class->get_snmp_info(
                        session   => $sinfo,
    ...
    unless ($dev) {    #still no dev! guess we better make it!
        $logger->debug(
            sub { "Device::discover: Device $name does not yet exist" } );
        ...
        my $newname =
          $class->assign_name( host => $host, sysname => $info->{sysname} );
        ...
        if ( !$info->{type} && $info->{layers} ) {
            ...
            $info->{type} = "Router"
              if ( $class->_layer_active( $info->{layers}, 3 )
                && $info->{ipforwarding} );
            $info->{type} = "Server"
              if ( $class->_layer_active( $info->{layers}, 7 ) );
            $info->{type} = "Unknown" unless defined $info->{type};
        }
        $logger->debug(
            sub {
                sprintf( "%s type is: %s", $newname->get_label, $info->{type} );
            }
        );
    ...
    my %uargs;
    foreach my $field (
        qw(communities version timeout retries
...
sub get_all_from_block {
    ...
    defined $block
      || $class->throw_fatal(
        "Model::Device::get_all_from_block: Missing required arguments: block");
...
sub get_base_macs_from_all {
    ...
    # Build the SQL query
    $logger->debug(
        sub { "Device::get_base_macs_from_all: Retrieving all Device MACs..." }
    );

    my $dbh  = $class->db_Main;
    my $aref = $dbh->selectall_arrayref(
        "SELECT p.address, d.id
...
sub get_if_macs_from_all {
    ...
    # Build the SQL query
    $logger->debug(
        sub { "Device::get_if_macs_from_all: Retrieving all Interface MACs..." }
    );
...
sub get_within_downtime {
    ...
    my @devices = Device->retrieve_from_sql(
        qq{
        down_from <= '$now'
        AND down_until >= '$now'
        }
...
sub get_ips_from_all {
    ...
    # Build the SQL query
    $logger->debug(
        sub { "Device::get_ips_from_all: Retrieving all Device IPs..." } );

    my $dbh   = $class->db_Main;
    my $aref1 = $dbh->selectall_arrayref(
        "SELECT ip.address, d.id
                                         FROM   ipblock ip, device d, interface i
                                         WHERE  i.device=d.id AND ip.interface=i.id
                                         "
    );

    my $aref2 = $dbh->selectall_arrayref(
        "SELECT ip.address, d.id
...
sub shortest_path_parents {
    ...
    $logger->debug(
"Netdot::Device::shortest_path_parents: Determining all shortest paths to Device id $s"
    );
        ...
        foreach my $n2 ( keys %{ $graph->{$n} } ) {
            ...
            if ( $dist{$n2} >= ( $dist{$n} + $cost{$n} ) ) {
                ...
                foreach my $p ( keys %{ $parents{$n2} } ) {
                    delete $parents{$n2}{$p}
                      if ( $dist{$p} + $cost{$p} > $dist{$n} + $cost{$n} );
                }
                $parents{$n2}{$n} = 1;
            }
        }
...
sub add_contact_lists {
    ...
    if ( !$argv ) {
        ...
        if ( my $default_cl = ContactList->search( name => $confcl )->first ) {
            ...
            $logger->warn(
                "add_contact_lists: Default Contact List not found: $confcl");
            return;
        }
    }
    else {
    ...
    foreach my $cl (@cls) {
        my $n =
          DeviceContacts->insert( { device => $self, contactlist => $cl } );
        push @ret, $n;
    }
    return @ret;
}
...
sub arp_update {
    ...
    unless ( $self->collect_arp ) {
        $logger->debug(
            sub {
                "Device::arp_update: $host: Collect ARP option off. Skipping";
            }
        );
        return;
    }
    if ( $self->is_in_downtime ) {
        $logger->debug(
            sub { "Device::arp_update: $host in downtime. Skipping" } );
        return;
    }
    ...
    my $cache =
      ...
      || $class->_exec_timeout( $host,
        sub { return $self->get_arp( session => $argv{session} ) } );
    ...
    if ( my $e = $@ ) {
        $logger->warn(
            sprintf(
                "Device %s: Could not insert ArpCache at %s: %s",
                $self->fqdn, $timestamp, $e
            )
        );
        return;
    }

    $self->_update_macs_from_arp_cache(
        caches    => [$cache],
        timestamp => $timestamp,
        atomic    => $argv{atomic},
    );

    $self->_update_ips_from_arp_cache(
        caches         => [$cache],
        timestamp      => $timestamp,
    ...
    if ( $argv{atomic} ) {
        Netdot::Model->do_transaction(
            sub { return ArpCacheEntry->fast_insert( list => \@ce_updates ) } );
    }
    else {
    ...
    $logger->debug(
        sub {
            sprintf( "$host: ARP cache updated. %s entries in %s",
...
sub get_arp {
    ...
    unless ( $self->collect_arp ) {
        $logger->debug(
            sub { "Device::get_arp: $host: Collect ARP option off. Skipping" }
        );
        return;
    }
    if ( $self->is_in_downtime ) {
    ...
    $logger->info(
        sub {
            sprintf( "$host: ARP cache fetched. %s entries in %s",
                $arp_count, $self->sec2dhms( $end - $start ) );
        }
    );
    ...
    if ( $self->config->get('GET_IPV6_ND') ) {
        ...
        $logger->info(
            sub {
                sprintf( "$host: IPv6 ND cache fetched. %s entries in %s",
...
sub fwt_update {
    ...
    unless ( $self->collect_fwt ) {
        $logger->debug(
            sub {
                "Device::fwt_update: $host: Collect FWT option off. Skipping";
            }
        );
        return;
    }
    if ( $self->is_in_downtime ) {
        $logger->debug(
            sub { "Device::fwt_update: $host in downtime. Skipping" } );
        return;
    }
    ...
    my $fwt = $argv{fwt}
      || $class->_exec_timeout( $host,
        sub { return $self->get_fwt( session => $argv{session} ) } );
    ...
    if ( my $e = $@ ) {
        $logger->warn(
            sprintf(
                "Device %s: Could not insert FWTable at %s: %s",
                $self->fqdn, $timestamp, $e
            )
        );
        return;
    }
    $self->_update_macs_from_fwt(
        caches    => [$fwt],
        timestamp => $timestamp,
        atomic    => $argv{atomic},
    );
    ...
    if ( $argv{atomic} ) {
        Netdot::Model->do_transaction(
            sub { return FWTableEntry->fast_insert( list => \@fw_updates ) } );
    }
    else {
    ...
    $logger->debug(
        sub {
            sprintf(
                "$host: FWT updated. %s entries in %s",
                scalar @fw_updates,
                $self->sec2dhms( $end - $start )
            );
        }
    );
...
sub get_fwt {
    ...
    unless ( $self->collect_fwt ) {
        $logger->debug(
            sub { "Device::get_fwt: $host: Collect FWT option off. Skipping" }
        );
        return;
    }
    if ( $self->is_in_downtime ) {
    ...
    $logger->info(
        sub {
            sprintf( "$host: FWT fetched. %s entries in %s",
...
sub short_name {
    ...
    my $rr;
    $self->throw_fatal( "Model::Device::short_name: Device id "
          . $self->id
          . " has no RR defined" )
      unless ( $rr = $self->name );
    if ($name) {
...
sub is_in_downtime {
    ...
    if (   $self->down_from
        && $self->down_until
        && $self->down_from ne '0000-00-00'
        && $self->down_until ne '0000-00-00' )
    {
...
sub update_bgp_peering {
    ...
    if ( $peer->{asname} || $peer->{orgname} || $peer->{asnumber} ) {
        ...
        if ( $entity = Entity->search_where( \@where )->first ) {
            ...
            $logger->info(
                sprintf(
                    "%s: Peer Entity %s not found. Inserting",
                    $host, $entityname
                )
            );
            ...
            $logger->info(
                sprintf( "%s: Created Peer Entity %s.", $host, $entityname ) );
        }
        ...
        if ( my $type = ( EntityType->search( name => "Peer" ) )[0] ) {
            ...
            if ( $erole = EntityRole->search(%eroletmp)->first ) {
                $logger->debug(
                    sub {
                        sprintf( "%s: Entity %s already has 'Peer' role",
                            $host, $entityname );
                    }
                );
            }
        ...
        $logger->warn(
            sprintf(
"%s: Missing peer info. Cannot associate peering %s with an entity",
                $host, $peer->{address}
            )
        );
    ...
    if ($p) {
        ...
        $logger->debug(
            sub {
                sprintf( "%s: Updated Peering with: %s. ",
                    $host, $entity->name );
            }
        ) if $r;
        ...
        $logger->info(
            sprintf(
                "%s: Inserted new Peering with: %s. ", $host, $peer_label
            )
        );
    }
    return $p;
...
sub snmp_update {
    ...
    my $atomic =
      defined $argv{atomic}
      ? $argv{atomic}
      : $self->config->get('ATOMIC_DEVICE_UPDATES');
    ...
    if ( $argv{do_fwt} ) {
        if ( $self->collect_fwt ) {
            ...
            $logger->debug(
                sub {
"Device::snmp_update: $host: Collect FWT option off. Skipping";
                }
            );
        }
    }
    if ( $argv{do_arp} ) {
        if ( $self->collect_arp ) {
            ...
            $logger->debug(
                sub {
"Device::snmp_update: $host: Collect ARP option off. Skipping";
                }
            );
        }
    }
...
sub info_update {
    ...
    unless ($info) {
        ...
        if ( $argv{session} ) {
            ...
            my $timeout =
              $argv{timeout} || $self->config->get('DEFAULT_SNMPTIMEOUT');
            my $retries =
              $argv{retries} || $self->config->get('DEFAULT_SNMPRETRIES');
            my $communities =
                 $argv{communities}
              || [ $self->community ]
    ...
    unless ( ref($info) eq 'HASH' ) {
        $self->throw_fatal(
            "Model::Device::info_update: Invalid info data structure");
    }
    ...
    if ( $argv{pretend} ) {
        ...
        unless ( Netdot::Model->db_auto_commit(0) == 0 ) {
            $self->throw_fatal(
                "Model::Device::info_update: Unable to set AutoCommit off");
        }
    }
    ...
    foreach my $field (
        qw( community layers ipforwarding sysname
    ...
    $asset_args{reserved_for} =
      "";    # needs to be cleared when device gets installed
    ...
    if ( $asset_sn && $asset_phy ) {
        if ( $asset_sn->id != $asset_phy->id ) {
            ...
            $logger->debug(
                sub {
                    sprintf( "%s: Deleting duplicate asset %s",
    ...
    if ( $argv{device_is_new} ) {
        ...
        $devtmp{snmp_version} = $info->{snmp_version}
          if exists $info->{snmp_version};
    ...
    #
    my $update_bgp =
      defined( $argv{bgp_peers} )
      ? $argv{bgp_peers}
      : $self->config->get('ADD_BGP_PEERS');

    if ($update_bgp) {
        $self->_update_bgp_info(
            bgp_local_as => $info->{bgplocalas},
            bgp_id       => $info->{bgpid},
            peers        => $info->{bgp_peer},
            newdev       => \%devtmp
        );
    ...
    $logger->debug(
        sub {
            sprintf( "%s: SNMP update completed in %s",
    ...
    if ( $argv{pretend} ) {
        ...
        if ( my $e = $@ ) {
            $self->throw_fatal(
                "Model::Device::info_update: Rollback Failed!: $e");
        }
        $logger->debug(
            sub { "Model::Device::info_update: Turning AutoCommit back on" } );
        unless ( Netdot::Model->db_auto_commit(1) == 1 ) {
            $self->throw_fatal(
                "Model::Device::info_update: Unable to set AutoCommit on");
        }
    }
...
sub get_ips {
    ...
    if ( $argv{sort_by} ) {
        if ( $argv{sort_by} eq "address" ) {
            ...
            $self->throw_fatal(
                "Model::Device::get_ips: Invalid sort criteria: $argv{sort_by}"
            );
        }
    }
    else {
        ...
        map { $_->[0] } sort {
          ...
          map {
            [
                $_,
                $_->name =~ /^(\D+)\d/,
                ( split( /\D+/, $_->name ) )[ 0, 1, 2, 3, 4, 5, 6, 7, 8 ]
            ]
          } @ifs
    );
...
sub ints_by_vlan {
    ...
    my @ifs = $self->interfaces();
    my @tmp =
      map { [ ( $_->vlans ) ? ( $_->vlans )[0]->vlan->vid : 0, $_ ] } @ifs;
    @ifs = map { $_->[1] } sort { $a->[0] <=> $b->[0] } @tmp;
...
sub ints_by_descr {
    ...
    my @ifs =
      Interface->search( device => $self->id, { order_by => 'description' } );
...
sub ints_by_monitored {
    ...
    my @ifs = Interface->search(
        device => $self->id,
        { order_by => 'monitored DESC' }
    );
...
sub ints_by_status {
    ...
    my @ifs =
      Interface->search( device => $self->id, { order_by => 'oper_status' } );
...
sub interfaces_by {
    ...
    if ( $sort eq "number" ) {
        ...
        $self->throw_fatal(
            "Model::Device::interfaces_by: Unknown sort field: $sort");
    }
}
...
sub bgppeers_by_entity {
    ...
    unless ( $sort =~ /^name|asnumber|asname$/o ) {
        $self->throw_fatal(
            "Model::Device::bgppeers_by_entity: Invalid Entity field: $sort");
    }
    my $sortsub =
      ( $sort eq "asnumber" )
...
sub get_bgp_peers {
    ...
    if ( $argv{entity} ) {
        ...
        if ( $argv{type} eq "internal" ) {
            ...
            $self->throw_fatal(
                "Model::Device::get_bgp_peers: Invalid type: $argv{type}");
        }
    }
    elsif ( !$argv{sort} ) {
        $self->throw_fatal(
            "Model::Device::get_bgp_peers: Missing or invalid search criteria");
    }
    if ( $argv{sort} =~ /entity|asnumber|asname/ ) {
        ...
        $self->throw_fatal(
            "Model::Device::get_bgp_peers: Invalid sort argument: $argv{sort}");
    }
...
sub set_overwrite_if_descr {
    ...
    $self->throw_fatal(
"Model::Device::set_overwrite_if_descr: Invalid value: $value.  Should be 0|1"
    ) unless ( $value =~ /^0|1$/ );
...
sub set_interfaces_auto_dns {
    ...
    $self->throw_fatal(
"Model::Device::set_interfaces_auto_dns: Invalid value: $value. Should be 0|1"
    ) unless ( $value =~ /^0|1$/ );
...
sub do_auto_dns {
    ...
    unless ( $self->auto_dns ) {
        $logger->debug(
            sub {
                sprintf(
                    "Auto DNS is disabled for device %s." . " Skipping.",
                    $host
                );
            }
        );
    ...
    $logger->debug(
        sub {
            sprintf( "Generating DNS records for IP interfaces in %s", $host );
        }
    );
    ...
    if ( @hostnameips = Netdot->dns->resolve_name($host) ) {
        $logger->debug(
            sub {
                sprintf( "Device::do_auto_dns: %s resolves to: %s",
    ...
    foreach my $ip (@my_ips) {
        ...
        if ( my $e = $@ ) {
            $logger->error(
                sprintf(
                    "Error updating A record for IP %s: %s",
                    $ip->address, $e
                )
            );
        }
...
sub _validate_args {
    ...
    if ( defined $args->{snmp_version} ) {
        if ( $args->{snmp_version} !~ /^1|2|3$/ ) {
            $class->throw_user(
                "Invalid SNMP version.  It must be either 1, 2 or 3");
        }
    }

    # Asset
    if ( $args->{asset_id}
        && ( my $asset = Asset->retrieve( int( $args->{asset_id} ) ) ) )
    {
        ...
        if ( my $otherdev = $asset->devices->first ) {
            if ( defined $self ) {
                if ( $self->id != $otherdev->id ) {
                    my $msg = sprintf(
                        "%s: Existing device: %s uses S/N %s, MAC %s",
                        $self->fqdn,           $otherdev->fqdn,
                        $asset->serial_number, $asset->physaddr
                    );
...
sub _layer_active {
    ...
    $class->throw_fatal(
"Model::Device::_layer_active: Missing required arguments: layers && layer"
    ) unless ( $layers && $layer );
...
sub _get_snmp_session {
    ...
    if ( $class = ref($self) ) {
        ...
        unless ( $self->snmp_managed ) {
            $logger->debug(
                sprintf( "Device %s not SNMP-managed. Aborting.", $self->fqdn )
            );
            return;
        }
        ...
        if ( $self->snmp_down == 1 ) {
            $logger->info(
                sprintf( "Device %s has been marked as snmp_down. Aborting.",
                    $self->fqdn )
            );
            return;
        }
        ...
        $self->throw_user(
            sprintf( "Could not determine IP nor hostname for Device id: %d",
                $self->id )
        ) unless $argv{host};
        ...
        $self->throw_fatal(
            "Model::Device::_get_snmp_session: Missing required arguments: host"
        ) unless $argv{host};
    }
    ...
    sub _check_max_attempts {
        ...
        $logger->info(
            sprintf(
                "Device::_get_snmp_session: %s: Failed connection attempts: %d",
                $host, $count
            )
        );
        if ( $max == 0 ) {
    ...
    if ( $sinfoargs{Version} == 3 ) {
        ...
        $logger->debug(
            sub {
                sprintf(
                    "Device::get_snmp_session: Trying SNMPv%d session with %s",
        ...
        if ( defined $sinfo ) {
            ...
            if ( my $err = $sinfo->error ) {
                $self->throw_user(
                    sprintf(
"Device::_get_snmp_session: SNMPv%d error: device %s: %s",
            ...
            $self->throw_user(
                sprintf(
                    "Device::get_snmp_session: %s: SNMPv%d failed",
        ...
        foreach my $community ( @{ $argv{communities} } ) {
            ...
            $logger->debug(
                sub {
                    sprintf(
"Device::_get_snmp_session: Trying SNMPv%d session with %s, "
                          . "community %s",
                        $sinfoargs{Version}, $argv{host},
                        $sinfoargs{Community} );
                }
            ...
            if ( !defined $sinfo && $sinfoargs{Version} == 2 ) {
                $logger->debug(
                    sub {
                        sprintf(
"Device::_get_snmp_session: %s: SNMPv%d failed. Trying SNMPv1",
                            $argv{host}, $sinfoargs{Version} );
                    }
                );
                $sinfoargs{Version} = 1;
            ...
            if ( defined $sinfo ) {
                ...
                if ( my $err = $sinfo->error ) {
                    $self->throw_user(
                        sprintf(
"Device::_get_snmp_session: SNMPv%d error: device %s, "
                              . "community '%s': %s",
                            $sinfoargs{Version},   $argv{host},
                ...
                last
                  ;  # If we made it here, we are fine.  Stop trying communities
                ...
                $logger->debug(
                    sub {
                        sprintf(
"Device::_get_snmp_session: Failed SNMPv%s session with "
                              . "%s community '%s'",
                            $sinfoargs{Version}, $argv{host},
                            $sinfoargs{Community} );
                    }
        ...
        unless ( defined $sinfo ) {
            ...
            $self->throw_user(
                sprintf(
                    "Device::_get_snmp_session: Cannot connect to %s. "
                      . "Tried communities: %s",
    ...
    if ($class) {
        ...
        $logger->debug(
            sub {
                "Device::get_snmp_session: $argv{host} is: ", $sinfo->class();
            }
        );
        ...
        $uargs{snmp_conn_attempts} = 0;
        $uargs{snmp_down}          = 0;

        # Fill out some SNMP parameters if they are not set
        $uargs{snmp_version} = $sinfoargs{Version}
          unless defined( $self->snmp_version );
        $uargs{snmp_bulk} = $sinfoargs{BulkWalk}
          unless defined( $self->snmp_bulk );

        if ( $sinfoargs{Version} == 3 ) {

            # Store v3 parameters
            $uargs{snmp_securityname} = $sinfoargs{SecName}
              unless defined( $self->snmp_securityname );
            $uargs{snmp_securitylevel} = $sinfoargs{SecLevel}
              unless defined( $self->snmp_securitylevel );
            $uargs{snmp_authprotocol} = $sinfoargs{AuthProto}
              unless defined( $self->snmp_authprotocol );
            $uargs{snmp_authkey} = $sinfoargs{AuthPass}
              unless defined( $self->snmp_authkey );
            $uargs{snmp_privprotocol} = $sinfoargs{PrivProto}
              unless defined( $self->snmp_privprotocol );
            $uargs{snmp_privkey} = $sinfoargs{PrivPass}
              unless defined( $self->snmp_privkey );

        }
        else {
            $uargs{community} = $sinfoargs{Community}
              unless defined( $self->community );
        }
        $self->update( \%uargs ) if ( keys %uargs );
    }
    $logger->debug(
        sub {
            sprintf( "SNMPv%d session with host %s established",
                $sinfoargs{Version}, $argv{host} );
        }
    );
    ...
    delete $munge->{'i_speed'}
      ;    # We store these as integers in the db.  Munge at display
    ...
    foreach my $m ( 'i_mac', 'fw_mac', 'mac', 'b_mac', 'at_paddr',
        'rptrAddrTrackNewLastSrcAddress', 'stp_p_port' )
    {
...
sub _get_main_ip {
    ...
    $class->throw_fatal(
        "Model::Device::_get_main_ip: Missing required argument (info)")
      unless $info;
    my @methods = @{ $class->config->get('DEVICE_NAMING_METHOD_ORDER') };
    $class->throw_fatal(
"Model::Device::_get_main_ip: Missing or invalid configuration variable: "
    ...
    map {
        map { $allips{$_} = '' } keys %{ $info->{interface}->{$_}->{ips} }
    } @allints;
    ...
    foreach my $method (@methods) {
        ...
        if ( $method eq 'sysname' && $info->{sysname} ) {
            ...
            my @ordered =
              sort { $loopbacks{$a} <=> $loopbacks{$b} } keys %loopbacks;
            my $main_int;
        ...
        if ( defined $ip ) {
            if ( Ipblock->matches_v4($ip) && Ipblock->validate($ip) ) {
                $logger->debug(
                    sub {
"Device::_get_main_ip: Chose $ip using naming method: $method";
                    }
                );
                return $ip;
            }
                ...
                $logger->debug(
                    sub { "Device::_get_main_ip: $ip not valid.  Ignoring" } );
    ...
    $logger->debug(
        sub {
"Device::_get_main_ip: Could not determine the main IP for this device";
        }
    );
    return;
}
...
sub _get_stp_info {
    ...
    my %res;
    foreach my $method (
        'stp_root',   'stp_root_port', 'stp_priority', 'i_stp_bridge',
        'i_stp_port', 'i_stp_state',
      )
    {
...
sub _get_i_stp_info {
      ...
      )
    {
        ...
        foreach my $i ( keys %{ $res{ $STPFIELDS{$method} } } ) {
            $res{ $STPFIELDS{$method} }->{$i} =
              (      $res{ $STPFIELDS{$method} }->{$i} eq 'true'
...
sub _get_devs_from_file {
    ...
    $class->throw_user(
            "Device::_get_devs_from_file: No existing devices in list. "
...
sub _get_hosts_from_file {
    ...
    $class->throw_user(
        "Device::_get_hosts_from_file: Missing or invalid file: $file")
      unless ( defined($file) && -r $file );

    open( FILE, "<$file" )
      or $class->throw_user("Can't open file $file for reading: $!");

    $logger->debug(
        sub { "Device::_get_hosts_from_file: Retrieving host list from $file" }
    );
...
sub _fork_init {
    ...
    unless ( $dbh->{InactiveDestroy} = 1 ) {
        $class->throw_fatal(
            "Model::Device::_fork_init: Cannot set InactiveDestroy: ",
            $dbh->errstr );
    }
    ...
    $logger->debug(
        sub {
            "Device::_fork_init: Launching up to $MAXPROCS children processes";
        }
    );
...
sub _snmp_update_parallel {
    ...
    if ( $use_sqe && !$Netdot::Model::Device::_sqe_module_loaded ) {
        ...
        eval {
            ...
            Coro::AnyEvent->import;
            require Netdot::FakeSNMPSession;
            Netdot::FakeSNMPSession->import;
        ...
        unless ($Netdot::Model::Device::_sqe_module_loaded) {
            $logger->info(
                "Failure loading SQE-related modules, disabling SQE: $@");
            $use_sqe = "";
        }
    }
    if ($use_sqe) {
        ...
        eval {
            $sqe = Net::SNMP::QueryEngine::AnyEvent->new(
                connect    => \@conn,
            ...
            if ($sqe) {
                $sqe->info(
                    sub { my ( $h, $ok, $r ) = @_; $sqe = undef unless $ok } );
                $sqe->wait;
            }
        };
    }
    if ($sqe) {
        $logger->info(
            "SQE is requested and available, using it for SNMP collection");
        $logger->debug(
            "$class\::_snmp_update_parallel: Loading dummy SNMP::Info object");
        my $dummy = SNMP::Info->new(
            DestHost    => 'localhost',
            Version     => 1,
            AutoSpecify => 0,
        ...
        if ($use_sqe) {
            $logger->info(
"SQE daemon is NOT available, using traditional method for SNMP collection"
            );
        }
        else {
...
sub _snmp_update_parallel_args_check {
    ...
    if ( defined $argv{hosts} ) {
        $class->throw_fatal(
            "Model::Device::_snmp_update_parallel: Invalid hosts hash")
          if ( ref( $argv{hosts} ) ne "HASH" );
        $hosts = $argv{hosts};
    }
    elsif ( defined $argv{devs} ) {
        $class->throw_fatal(
            "Model::Device::_snmp_update_parallel: Invalid devs array")
          if ( ref( $argv{devs} ) ne "ARRAY" );
        $devs = $argv{devs};
    }
    else {
        $class->throw_fatal(
"Model::Device::_snmp_update_parallel: Missing required parameters: "
              . "hosts or devs" );
    }

    my %uargs;
    foreach my $field (
        qw(version timeout retries sec_name sec_level auth_proto auth_pass
...
sub _snmp_update_get_device_args {
    ...
    if ( $args{do_info} ) {
        unless ( $dev->canautoupdate ) {
            $logger->debug(
                sub { sprintf( "%s: Auto Update option off", $dev->fqdn ) } );
            $args{do_info} = 0;
        }
    }
    if ( $args{do_fwt} ) {
        unless ( $dev->collect_fwt ) {
            $logger->debug(
                sub { sprintf( "%s: Collect FWT option off", $dev->fqdn ) } );
            $args{do_fwt} = 0;
        }
    }
    if ( $args{do_arp} ) {
        unless ( $dev->collect_arp ) {
            $logger->debug(
                sub { sprintf( "%s: Collect ARP option off", $dev->fqdn ) } );
            $args{do_arp} = 0;
        }
    }
    ...
    if ($devs) {
        ...
        foreach my $host ( keys %$hosts ) {
            ...
            if ( $dev = $class->search( name => $host )->first ) {
                $do_devs{ $dev->id } = $dev;
                $logger->debug(
                    sub { sprintf( "%s exists in DB.", $dev->fqdn ) } );
            }
            else {
                $device_count++;
                $n_polling_devices++;
    ...
    while ( my ( $id, $dev ) = each %do_devs ) {
        ...
        if ( $dev->is_in_downtime() ) {
            $logger->debug(
                sub {
                    sprintf(
"Model::Device::_snmp_update_parallel_sqe: %s in downtime.  Skipping",
                        $dev->fqdn );
                }
            );
            next;
            ...
            if ( $dev = $class->search( name => $host )->first ) {
                ...
                eval {
                    $class->_launch_child(
                        pm   => $pm,
                        code => sub {
                            return $class->discover( name => $host, %uargs );
                        }
                    );
                };
                if ( my $e = $@ ) {
    ...
    while ( my ( $id, $dev ) = each %do_devs ) {
        ...
        if ( $dev->is_in_downtime() ) {
            $logger->debug(
                sub {
                    sprintf(
"Model::Device::_snmp_update_parallel_traditional: %s in downtime. Skipping",
                        $dev->fqdn );
                }
            );
            next;
...
sub _update_poll_stats {
    ...
    my @vals = (
        $stats->{ips}, $stats->{macs}, $stats->{arp_devices},
...
sub _get_poll_stats {
    ...
    # MAC addresses
    my $sth3 = $dbh->prepare(
        'SELECT COUNT(DISTINCT i.physaddr)
...
sub _get_arp_from_snmp {
    ...
    if ( $paddr_keys[0] && $paddr_keys[0] =~ /^(\d+)(\.1|\.4)?\.($IPV4)$/ ) {
        ...
        if ( !exists $devints{$idx} ) {
            ...
            $logger->debug(
                sub {
"Device::_get_arp_from_snmp: $host: Not using ARP query shortcut";
                }
            );
    ...
    foreach my $key (@paddr_keys) {
        ...
        if ($use_shortcut) {
            if ( $key =~ /^(\d+)(\.1|\.4)?\.($IPV4)$/ ) {
                ...
                $logger->debug(
                    sub {
"Device::_get_arp_from_snmp: $host: Unrecognized hash key: $key";
                    }
                );
                next;
            }
        ...
        unless ( $ip && $idx && $mac ) {
            $logger->debug(
                sub {
"Device::_get_arp_from_snmp: $host: Missing information at row: $key";
                }
            );
            next;
        }
...
sub _get_v6_nd_from_snmp {
    ...
    unless ( $sinfo->can('ipv6_n2p_mac') ) {
        $logger->debug(
                "Device::_get_v6_nd_from_snmp: This version of SNMP::Info "
        ...
        && %$n2p_if )
    {
        $logger->debug(
            sub {
                "Device::_get_v6_nd_from_snmp: $host: No IPv6 ND information";
            }
        );
        return;
    }
    while ( my ( $row, $val ) = each %$n2p_mac ) {
        ...
        unless ( $idx && $ip && $mac ) {
            $logger->debug(
                sub {
"Device::_get_v6_nd_from_snmp: $host: Missing information in row: $row";
                }
            );
            next;
        }
...
sub _validate_arp {
    ...
    foreach my $idx ( keys %$cache ) {
        ...
        unless ($intid) {
            $logger->warn(
"Device::_validate_arp: $host: Interface $idx not in database. Skipping"
            );
            next;
        }
        foreach my $ip ( keys %{ $cache->{$idx} } ) {
            if (   $version == 6
                && Ipblock->is_link_local($ip)
                && Netdot->config->get('IGNORE_IPV6_LINK_LOCAL') )
            {
                next;
            }
            my $mac = $cache->{$idx}->{$ip};
            eval { $mac = PhysAddr->validate($mac); };
            if ( my $e = $@ ) {
                $logger->debug(
                    sub { "Device::_validate_arp: $host: Invalid MAC: $e" } );
                next;
            }
            if ($ign_non_subnet) {
              ...
              # just validate them
                ...
                unless ( $nip = NetAddr::IP->new($ip) ) {
                    $logger->error(
"Device::_validate_arp: Cannot create NetAddr::IP object from $ip"
                    );
                    next;
                }
            ...
            $logger->debug(
                sub {
                    "Device::_validate_arp: $host: valid: $idx -> $ip -> $mac";
                }
            );
        }
    }
...
sub _get_fwt_from_snmp {
    ...
    unless ( $self->collect_fwt ) {
        $logger->debug(
            sub {
"Device::_get_fwt_from_snmp: $host: Collect FWT option off. Skipping";
            }
        );
        return;
    }
    ...
    if ( $self->is_in_downtime ) {
        $logger->debug(
            sub { "Device::_get_fwt_from_snmp: $host in downtime. Skipping" } );
        return;
    }
    ...
    if ($cisco_comm_indexing) {
        $logger->debug(
            sub {
"$host supports Cisco community string indexing. Connecting to each VLAN";
            }
        );
        ...
        foreach my $vlan ( $self->_get_active_vlans( sinfo => $sinfo ) ) {
            ...
            my $vlan_sinfo;
            eval {
                $vlan_sinfo = $self->_get_cisco_snmp_context_session(
                    sinfo => $sinfo,
                    vlan  => $vlan
                );
            };
    ...
    $logger->debug(
        sub {
            sprintf( "$host: FWT fetched. %d entries in %s",
                $fwt_count, $self->sec2dhms( $end - $start ) );
        }
    );
...
sub _walk_fwt {
    ...
    my ( $sinfo, $sints, $devints, $fwt ) =
      @argv{ "sinfo", "sints", "devints", "fwt" };
    ...
    if ( my $fw_mac = $sinfo->fw_mac() ) {
        ...
        foreach my $fw_index ( keys %$fw_mac ) {
            ...
            unless ( defined $mac ) {
                $logger->debug(
                    sub {
"Device::_walk_fwt: $host: MAC not defined at index $fw_index. Skipping";
                    }
                );
                next;
            }
            ...
            unless ( defined $bp_id ) {
                $logger->debug(
                    sub {
"Device::_walk_fwt: $host: Port $fw_index has no fw_port mapping. Skipping";
                    }
                );
                next;
            }
            ...
            unless ( defined $iid ) {
                $logger->debug(
                    sub {
"Device::_walk_fwt: $host: Interface $bp_id has no bp_index mapping. Skipping";
                    }
                );
                next;
            }
        ...
        foreach my $iid ( keys %{$last_src} ) {
            ...
            unless ( defined $mac ) {
                $logger->debug(
                    sub {
"Device::_walk_fwt: $host: MAC not defined at rptr index $iid. Skipping";
                    }
                );
                next;
            }
    ...
    foreach my $iid ( keys %tmp ) {
        ...
        unless ( defined $descr ) {
            $logger->debug(
                sub {
"Device::_walk_fwt: $host: SNMP iid $iid has no physical port matching. Skipping";
                }
            );
            next;
        }
        ...
        unless ($intid) {
            $logger->warn(
"Device::_walk_fwt: $host: Interface $iid ($descr) is not in database. Skipping"
            );
            next;
        }
        ...
        foreach my $mac ( keys %{ $tmp{$iid} } ) {
            ...
            if ( my $e = $@ ) {
                $logger->debug(
                    sub { "Device::_walk_fwt: $host: Invalid MAC: $e" } );
                next;
            }
            $fwt->{$intid}->{$mac} = 1;
            $logger->debug(
                sub { "Device::_walk_fwt: $host: $iid ($descr) -> $mac" } );
        }
    }
}
...
sub _exec_timeout {
    ...
    $class->throw_fatal(
        "Model::Device::_exec_timeout: Missing required argument: code")
      unless $code;
...
sub _launch_child {
    ...
    $class->throw_fatal(
        "Model::Device::_launch_child: Missing required arguments")
    ...
    unless ( $dbh->{InactiveDestroy} = 1 ) {
        $class->throw_fatal(
            "Model::Device::_launch_child: Cannot set InactiveDestroy: ",
            $dbh->errstr );
    }
...
sub _get_as_info {
    ...
    if ( my $e = $@ ) {
        $self->throw_fatal(
            "Model::Device::_get_as_info: Error loading module: $e");
    }
    ...
    if ( $obj =~ /as-name:\s+(.*)\n/o ) {
        ...
        $logger->debug(
            sub { "Device::_get_as_info:: $server: Found asname: $as_name" } );
    }
    if ( $obj =~ /descr:\s+(.*)\n/o ) {
        my $descr = $1;
        ...
        $logger->debug(
            sub { "Device::_get_as_info:: $server: Found orgname: $descr" } );
    }
    $results{orgname} ||= $asn;
...
sub _update_macs_from_fwt {
    ...
    if ($atomic) {
        Netdot::Model->do_transaction(
            sub { return PhysAddr->fast_update( \%mac_updates, $timestamp ) } );
    }
    else {
...
sub _update_ips_from_arp_cache {
    my ( $class, %argv ) = @_;
    my ( $caches, $timestamp, $no_update_tree, $atomic ) =
      @argv{ 'caches', 'timestamp', 'no_update_tree', 'atomic' };

    my %ip_updates;

    my $ip_status = ( IpblockStatus->search( name => 'Discovered' ) )[0];
    $class->throw_fatal(
"Model::Device::_update_ips_from_cache: IpblockStatus 'Discovered' not found?"
    ) unless $ip_status;
    ...
    if ($atomic) {
        Netdot::Model->do_transaction(
            sub { return Ipblock->fast_update( \%ip_updates ) } );
    }
    else {
...
sub _assign_base_mac {
    ...
    eval { $address = PhysAddr->validate($address); };
    if ( my $e = $@ ) {
        ...
        foreach my $iid ( sort { $a <=> $b } keys %{ $info->{interface} } ) {
            if ( my $addr = $info->{interface}->{$iid}->{physaddr} ) {
                eval { $address = PhysAddr->validate($addr); };
                if ( my $e = $@ ) {
                    $logger->debug( sub { "$host: Invalid MAC: $e" } );
                    next;
                }
    ...
    if ( $mac = PhysAddr->search( address => $address )->first ) {
        ...
        $logger->debug(
            sub { "$host: Using existing $address as base bridge address" } );
        return $mac;
    }
    else {
        ...
        if ( my $e = $@ ) {
            $logger->debug(
                sprintf(
                    "%s: Could not insert base MAC: %s: %s",
                    $host, $address, $e
                )
            );
        }
            ...
            $logger->info(
                sprintf(
                    "%s: Inserted new base MAC: %s", $host, $mac->address
                )
            );
            return $mac;
        }
...
sub _assign_snmp_target {
    ...
    if ( $self->snmp_managed && !$self->snmp_target && $info->{snmp_target} ) {
        ...
        unless ($ipb) {
            eval {
                $ipb = Ipblock->insert(
                    { address => $info->{snmp_target}, status => 'Static' } );
            };
            if ( my $e = $@ ) {
                $logger->warn(
"Device::assign_snmp_target: $host: Could not insert snmp_target address: "
                      . $info->{snmp_target} . ": ",
                    $e
                );
            }
        ...
        if ($ipb) {
            $logger->info(
                sprintf(
                    "%s: SNMP target address set to %s",
                    $host, $ipb->address
                )
            );
            return $ipb;
...
sub _assign_product {
    ...
    my @where;
    push @where, { sysobjectid => $info->{sysobjectid} }
      if $info->{sysobjectid};
...
sub _update_stp_info {
    ...
    $devtmp->{stp_enabled} = 1
      if ( defined $info->{stp_type} && $info->{stp_type} ne 'unknown' );
        ...
        if ( exists $info->{$field} ) {
            ...
            if ( $field eq 'stp_mst_region' || $field eq 'stp_mst_digest' ) {
                if ( defined( $self->$field )
                    && ( $self->$field ne $devtmp->{$field} ) )
                {
                    $logger->warn(
                        sprintf(
                            "%s: $field has changed: %s -> %s",
    ...
    if ( $devtmp->{stp_enabled} ) {
        ...
        $logger->debug(
            sub { sprintf( "%s: STP type is: %s", $host, $devtmp->{stp_type} ) }
        );
        ...
        foreach my $instn ( keys %{ $info->{stp_instances} } ) {
            ...
            my %uargs;
            if ( my $root_bridge =
                $info->{stp_instances}->{$instn}->{stp_root} )
            {
                if ( defined $stpinst->root_bridge
                    && ( $root_bridge ne $stpinst->root_bridge ) )
                {
                    $logger->warn(
                        sprintf(
"%s: STP instance %s: Root Bridge changed: %s -> %s",
                            $host,                 $stpinst->number,
                            $stpinst->root_bridge, $root_bridge
                        )
                    );
                }
                ...
                $logger->debug(
                    sub {
"$host: STP Designated Root not defined for instance $instn";
                    }
                );
            }

            if ( my $root_p =
                $info->{stp_instances}->{$instn}->{stp_root_port} )
            {
                if (   defined $stpinst->root_port
                    && $stpinst->root_port != 0
                    && ( $root_p != $stpinst->root_port ) )
                {
                    ...
                    $logger->warn(
                        sprintf(
                            "%s: STP instance %s: Root Port changed: %s -> %s",
                            $host,               $stpinst->number,
                            $stpinst->root_port, $root_p
                        )
                    );
                }
                ...
                $logger->debug(
                    sub {
                        "$host: STP Root Port not defined for instance $instn";
                    }
                );
            }
            ...
            if (   defined $stpinst->bridge_priority
                && defined $uargs{bridge_priority}
                && $stpinst->bridge_priority ne $uargs{bridge_priority} )
            {
                $logger->warn(
                    sprintf(
"%s: STP instance %s: Bridge Priority Changed: %s -> %s",
                        $host,                     $stpinst->number,
        ...
        if ( my @instances = $self->stp_instances() ) {
            $logger->debug(
                sub {
"$host: STP appears disabled.  Removing all existing STP instances";
                }
            );
...
sub _update_modules {
    ...
    foreach my $key ( keys %{$modules} ) {
        ...
        if ( exists $oldmodules{$number} ) {
            ...
            # Create new object
            $logger->info(
                "$host: New module $number ($show_name) found. Inserting.");
...
sub _update_interfaces {
    ...
    if ( defined $info->{sysobjectid}
        && exists $IGNORED{ $info->{sysobjectid} } )
    {
        $logger->debug(
            sub {
"Device::_update_interfaces: $host ports ignored per configuration option (IGNOREPORTS)";
            }
        );
        return;
    }
    ...
    if ( $info->{ipforwarding} ) {
        ...
        $add_subnets =
          ( defined( $argv{add_subnets} ) )
          ? $argv{add_subnets}
          : $self->config->get('ADDSUBNETS');
        $subs_inherit =
          ( $add_subnets && defined( $argv{subs_inherit} ) )
          ? $argv{subs_inherit}
          : $self->config->get('SUBNET_INHERIT_DEV_INFO');
    }
    ...
    if ( $ENV{REMOTE_USER} eq 'netdot' ) {
        ...
        if ( $int_thold <= 0 || $int_thold >= 1 ) {
            $self->throw_fatal(
                'Incorrect value for IF_COUNT_THRESHOLD in config file');
        }
          ...
          )
        {
            $logger->warn(
                sprintf(
"%s: new/old interface ratio: %.2f is below INT_COUNT_THRESHOLD"
          ...
          )
        {
            $logger->warn(
                sprintf(
                    "%s: new/old IP ratio: %.2f is below INT_COUNT_THRESHOLD"
                      . "Skipping interface update. Re-discover manually if needed.",
    ...
    foreach my $newif ( sort keys %{ $info->{interface} } ) {
        ...
        if ( $ifkey eq 'name' ) {
            if ( defined $newname && ( $oldif = $oldifsbyname{$newname} ) ) {
                ...
                $logger->debug(
                    sub {
                        sprintf( "%s: Interface with name %s found",
                            $host, $oldif->name );
                    }
                );
                ...
                if ( $oldif->number ne $newnumber ) {
                    ...
                    $logger->info(
                        sprintf(
                            "%s: Interface %s had number: %s, now has: %s",
                ...
                $logger->debug(
                    sub {
                        sprintf( "%s: Interface with number %s found",
                            $host, $oldif->number );
                    }
                );
            }
        ...
        if ( $if = $oldif ) {
            ...
            if ( defined $IFM ) {
                if ( $IFM == 0 ) {
                    ...
                    $logger->warn(
                        "Configured IF_MONITORED value: $IFM not recognized");
                }
            }
    ...
    foreach my $id ( keys %oldifs ) {
        ...
        if ( $iface->doc_status eq "snmp" ) {
            $logger->info(
                sprintf(
                    "%s: Interface %s no longer exists.  Marking as removed.",
    ...
    while ( my ( $address, $obj ) = each %old_ips ) {
        ...
        if ( $obj->interface && $obj->interface->ignore_ip ) {
            $logger->debug(
                sub {
                    sprintf( "%s: IP %s not deleted: %s set to Ignore IP",
            ...
            && $self->snmp_target->id == $obj->id )
        {
            $logger->debug(
                sub {
                    sprintf( "%s: IP %s is snmp target. Skipping delete",
                        $host, $obj->address );
                }
            );
            next;
        ...
        $logger->info(
            sprintf(
                "%s: IP %s no longer exists.  Removing.",
                $host, $obj->address
            )
        );
...
sub _update_bgp_info {
    ...
    if ( defined $argv{bgp_local_as} ) {
        $logger->debug(
            sub {
                sprintf( "%s: BGP Local AS is %s", $host, $argv{bgp_local_as} );
            }
        );
    ...
    if ( defined $argv{bgp_id} ) {
        $logger->debug(
            sub { sprintf( "%s: BGP ID is %s", $host, $argv{bgp_id} ) } );
    ...
    if ( $ENV{REMOTE_USER} eq 'netdot' ) {
     ...
     # to allow a real user to manually update it
          ...
          )
        {
            $logger->warn(
                sprintf(
                    "%s: new/old interface ratio: %.2f is below threshold (%s) "
    ...
    foreach my $peerid ( keys %old_peerings ) {
        ...
        $logger->info(
            sprintf(
                "%s: BGP Peering with %s (%s) no longer exists.  Removing.",
...
sub _netdot_rebless {
    ...
    if ( $class ne 'Device' && $class ne __PACKAGE__ ) {
        ...
        $logger->debug(
            "Device::_netdot_rebless: $host: Already reblessed as $class");
        return $self;
    }
    ...
    if ( defined $sclass && $sclass =~ /Airespace/o ) {
        $new_class .= '::Airespace';
        $logger->debug(
            "Device::_netdot_rebless: $host: changed class to $new_class");
    ...
    unless ($oid) {
        $logger->debug(
            "Device::_netdot_rebless: $host: sysObjectID not available");
        return;
    }
    my $obj = $self->_snmp_translate($oid);
    unless ($obj) {
        $logger->debug(
            "Device::_netdot_rebless: $host: Unknown SysObjectID $oid");
        return;
    }
