package Netdot::Model::Device::CLI::HuaweiVRP;
...
my $HUAWEI_MAC = '\w{4}\-\w{4}\-\w{4}';

=head1 NAME

Netdot::Model::Device::CLI::HuaweiVRP - HuaWei VRP Class
...
sub get_arp {
	...
	$logger->debug(sub{"Device::HuaweiVRP::_get_arp: $host excluded from ARP collection. Skipping"});
	return;
    }
    if ( $self->is_in_downtime ){
	$logger->debug(sub{"Device::HuaweiVRP::_get_arp: $host in downtime. Skipping"});
	return;
    }
...
sub get_fwt {
	...
	$logger->debug(sub{"Device::HuaweiVRP::get_fwt: $host excluded from FWT collection. Skipping"});
	return;
    }
    if ( $self->is_in_downtime ){
	$logger->debug(sub{"Device::HuaweiVRP::get_fwt: $host in downtime. Skipping"});
	return;
    }
...
sub _get_arp_from_cli {
    ...
    my @output = $self->_cli_cmd(%$args, host=>$host, cmd=>'disp arp all | no-more');

    my %cache;
    shift @output; # Ignore header line
    shift @output; # Ignore second line
    shift @output; # Ignore ------ line
    # Lines look like this:
    # Internet  10.82.250.129           -   0000.0c9f.f002  ARPA   GigabitEthernet0/3.2335
    foreach my $line ( @output ) {
	my ($iname, $ip, $mac, $intid);
	chomp($line);
	if ( $line =~ /($IPV4)\s+[-\d]+\s+($HUAWEI_MAC)\s+[I -|D-\d+]\s+(\S+)/o ) {
	    $ip    = $1;
	    $mac   = $2;
	    $iname = $3;
	}else{
	    $logger->debug(sub{"Device::CLI::HuaweiVRP::_get_arp_from_cli: line did not match criteria: $line" });
	    next;
	}
	unless ( $ip && $mac && $iname ){
	    $logger->debug(sub{"Device::HuaweiVRP::_get_arp_from_cli: Missing information: $line" });
	    next;
	}
	$cache{$iname}{$ip} = $mac;
    }
...
sub _get_v6_nd_from_cli {
    ...
    my @output = $self->_cli_cmd(%$args, host=>$host, cmd=>'disp ipv6 neighbors | no-more');
    shift @output; # Ignore header line
    my %cache;
    foreach my $line ( @output ) {
	my ($ip, $mac, $iname);
	...
	if ( $line =~ /^($IPV6)\s+\d+\s+($HUAWEI_MAC)\s+\S+\s+(\S+)/o ) {
	    $ip    = $1;
	    $mac   = $2;
	    $iname = $3;
	}else{
	    $logger->debug(sub{"Device::CLI::HuaweiVRP::_get_v6_nd_from_cli: line did not match criteria: $line" });
	    next;
	}
	unless ( $iname && $ip && $mac ){
	    $logger->debug(sub{"Device::HuaweiVRP::_get_v6_nd_from_cli: Missing information: $line"});
	    next;
	}
	$cache{$iname}{$ip} = $mac;
    }
...
#                                           VLAN/CEVLAN PVC
# ------------------------------------------------------------------------------
# 208.65.217.161  1047-80b1-229c            I -         GE0/3/1.999      DIA
# 208.65.217.160  Incomplete      1         D-9         GE0/3/1.999      DIA
# 208.65.217.162  001e-4a4c-1610  20        D-9         GE0/3/1.999      DIA

    foreach my $line ( @output ) {
	chomp($line);
	if ( $line =~ /^.*($HUAWEI_MAC)\s+dynamic\s+\S+\s+\S+\s+(\S+)\s*$/o ) {
	    $mac   = $1;
	    $iname = $2;
	}else{
	    $logger->debug(sub{"Device::CLI::HuaweiVRP::_get_fwt_from_cli: line did not match criteria: '$line'" });
	    next;
	}
	...
	unless ( $intid ) {
	    $logger->warn("Device::CLI::HuaweiVRP::_get_fwt_from_cli: $host: Could not match $iname to any interface names");
	    next;
	}
	...
	if ( $validmac ){
	    ...
	    $logger->debug(sub{"Device::CLI::HuaweiVRP::_get_fwt_from_cli: $host: Invalid MAC: $mac" });
	    next;
	}	

	# Store in hash
	$fwt{$intid}{$mac} = 1;
	$logger->debug(sub{"Device::CLI::HuaweiVRP::_get_fwt_from_cli: $host: $iname -> $mac" });
    }
