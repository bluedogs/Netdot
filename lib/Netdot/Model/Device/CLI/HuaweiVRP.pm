...
sub get_arp {
    ...
    unless ( $self->collect_arp ) {
        $logger->debug(
            sub {
                "Device::HuaweiVRP::_get_arp: $host excluded from ARP collection. Skipping"
            }
        );
        return;
    }
    if ( $self->is_in_downtime ) {
        $logger->debug(
            sub {"Device::HuaweiVRP::_get_arp: $host in downtime. Skipping"}
        );
        return;
    }
...
sub get_fwt {
    ...
    unless ( $self->collect_fwt ) {
        $logger->debug(
            sub {
                "Device::HuaweiVRP::get_fwt: $host excluded from FWT collection. Skipping"
            }
        );
        return;
    }
    if ( $self->is_in_downtime ) {
        $logger->debug(
            sub {"Device::HuaweiVRP::get_fwt: $host in downtime. Skipping"} );
        return;
    }
...
sub _get_arp_from_cli {
    ...
    my @output = $self->_cli_cmd(
        %$args,
        host => $host,
        cmd  => 'disp arp all | no-more'
    );
    ...
    foreach my $line (@output) {
        ...
        chomp($line);
        if ( $line
            =~ /($IPV4)\s+[-\d]+\s+($HUAWEI_MAC)\s+[I -|D-\d+]\s+(\S+)/o )
        {
            $ip    = $1;
            $mac   = $2;
            $iname = $3;
            ...
            $logger->debug(
                sub {
                    "Device::CLI::HuaweiVRP::_get_arp_from_cli: line did not match criteria: $line"
                }
            );
            next;
        }
        unless ( $ip && $mac && $iname ) {
            $logger->debug(
                sub {
                    "Device::HuaweiVRP::_get_arp_from_cli: Missing information: $line"
                }
            );
            next;
        }
...
sub _get_v6_nd_from_cli {
    ...
    my @output = $self->_cli_cmd(
        %$args,
        host => $host,
        cmd  => 'disp ipv6 neighbors | no-more'
    );
    shift @output;    # Ignore header line
    my %cache;
    foreach my $line (@output) {
 ...
 # FE80::219:E200:3B7:1920                     0 0019.e2b7.1920  REACH Gi0/2.3
        if ( $line =~ /^($IPV6)\s+\d+\s+($HUAWEI_MAC)\s+\S+\s+(\S+)/o ) {
            ...
            $logger->debug(
                sub {
                    "Device::CLI::HuaweiVRP::_get_v6_nd_from_cli: line did not match criteria: $line"
                }
            );
            next;
        }
        unless ( $iname && $ip && $mac ) {
            $logger->debug(
                sub {
                    "Device::HuaweiVRP::_get_v6_nd_from_cli: Missing information: $line"
                }
            );
            next;
        }
...
# 208.65.217.162  001e-4a4c-1610  20        D-9         GE0/3/1.999      DIA
    ...
    foreach my $line (@output) {
        ...
        if ( $line =~ /^.*($HUAWEI_MAC)\s+dynamic\s+\S+\s+\S+\s+(\S+)\s*$/o )
        {
            ...
            $logger->debug(
                sub {
                    "Device::CLI::HuaweiVRP::_get_fwt_from_cli: line did not match criteria: '$line'"
                }
            );
            next;
        }
        ...
        unless ($intid) {
            $logger->warn(
                "Device::CLI::HuaweiVRP::_get_fwt_from_cli: $host: Could not match $iname to any interface names"
            );
            next;
        }
        ...
        if ($validmac) {
            ...
            $logger->debug(
                sub {
                    "Device::CLI::HuaweiVRP::_get_fwt_from_cli: $host: Invalid MAC: $mac"
                }
            );
            next;
        }
        ...
        $logger->debug(
            sub {
                "Device::CLI::HuaweiVRP::_get_fwt_from_cli: $host: $iname -> $mac"
            }
        );
    }
